...
class PiecewiseRationalLinearCDF(transforms.Transform):
    def __init__(self,
                 shape,
                 num_bins=10,
                 tails=None,
                 tail_bound=1.,
                 identity_init=False,
                 min_bin_width=splines.rational_linear.DEFAULT_MIN_BIN_WIDTH,
                 min_bin_height=splines.rational_linear.DEFAULT_MIN_BIN_HEIGHT,
                 min_derivative=splines.rational_linear.DEFAULT_MIN_DERIVATIVE):
        super().__init__()
    ...
    def _spline(self, inputs, inverse=False):
        ...
        unnormalized_widths=_share_across_batch(self.unnormalized_widths, batch_size)
        unnormalized_heights=_share_across_batch(self.unnormalized_heights, batch_size)
        unnormalized_lambdas=_share_across_batch(self.unnormalized_lambdas, batch_size)
        unnormalized_derivatives=_share_across_batch(self.unnormalized_derivatives, batch_size)
