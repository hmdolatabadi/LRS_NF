...
class PiecewiseRationalLinearCouplingTransform(PiecewiseCouplingTransform):
    def __init__(self, mask, transform_net_create_fn,
                 num_bins=10,
                 tails=None,
                 tail_bound=1.,
        ...
        if apply_unconditional_transform:
            unconditional_transform = lambda features: transforms.PiecewiseRationalLinearCDF(
                shape=[features] + (img_shape if img_shape else []),
                num_bins=num_bins,
                tails=tails,
                tail_bound=tail_bound,
