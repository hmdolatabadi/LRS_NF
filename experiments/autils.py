...
def eval_log_density(log_prob_fn, classifier, data_loader, device, num_batches=None):
    with torch.no_grad():
        total_ld = 0
        batch_counter = 0
        total = 0
        ...
        for batch in data_loader:
            if isinstance(batch, list): # If labelled dataset, ignore labels
                ...
                batch = batch[0]
            noise, log_prob = log_prob_fn(batch)

            logits   = classifier(noise)
            crossnet = criterion(logits, label)
            ...
            if (num_batches is not None) and batch_counter == num_batches:
                break
        return total_ld / batch_counter, correct / total, crossnet
...
def eval_log_density_3(log_prob_fn, data_loader, num_batches=None):
    with torch.no_grad():
        total_ld = 0
        batch_counter = 0
