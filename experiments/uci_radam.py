...
# data
parser.add_argument('--dataset_name', type=str, default='hepmass',
...
# flow details
parser.add_argument('--base_transform_type', type=str, default='rl-autoregressive',
                    choices=['affine-coupling', 'quadratic-coupling', 'rq-coupling',
                             'affine-autoregressive', 'quadratic-autoregressive',
                             'rq-autoregressive', 'rl-coupling', 'rl-autoregressive'],
...
# create optimizer
# changed HERE
optimizer = RAdam(flow.parameters(), lr=args.learning_rate)
    ...
    if (step + 1) % args.monitor_interval == 0:
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                _, log_density_val = flow.log_prob(val_batch.to(device).detach())
