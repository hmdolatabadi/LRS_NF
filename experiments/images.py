...
def config():
    ...
    # Coupling transform
    coupling_layer_type = 'rational_linear_spline'
    spline_params = {
        'num_bins': 4,
        'tail_bound': 1.,
        'min_bin_width': 1e-3,
...
@ex.command(unobserved=True)
def sample_time(seed, iter_num, num_samples, _log):
    torch.set_grad_enabled(False)
    ...
    total_time = np.zeros((iter_num, ))

    for n in range(iter_num):
        t_start = time.time()
        _, _    = flow.sample_and_log_prob(num_samples)
        iter_time        = time.time() - t_start
        total_time[n]    = iter_time
        if iter_num%1000 == 0:
            _log.info('Iteration {}, Time: {:.4f}'.format(n, iter_time))

    _log.info('Avg. Generation Time For Generating {} Samples: {:.4f}'.format(num_samples, np.squeeze(np.mean(total_time))))
